...
from sensors import BeaconSensor, PinholeCamera2D, ExtendedCamera2D
...
pts[0,0] = start_pt[0];
pts[1,0] = start_pt[1];
#plt.plot(pts[0,0], pts[1,0], 'o', color='b', markersize=9)
for t in xrange(1,T):
  if t < 10:
    pts[:,t] = pts[:,t-1]
...
#ax = plt.subplot(122)
#s.draw(ax=ax)
localizer.draw_trajectory(mat2tuple(opt_X.T), mus=opt_mus,
        Sigmas=opt_Sigmas[0:2,0:2,:], color='blue')
localizer.draw_trajectory([], mus=opt_mus[2:4,0:1],
        Sigmas=opt_Sigmas[2:4,2:4,0:1], color='blue')
localizer.draw_trajectory([], mus=opt_mus[2:4,T-2:T-1],
        Sigmas=opt_Sigmas[2:4,2:4,T-2:T-1], color='blue')
