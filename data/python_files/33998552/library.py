...
def library_get_data(data, *args):
    ...
    retlist = [unicode(data[name_to_index[arg]])\
            if data[name_to_index[arg]] else None for arg in args]
    if len(retlist) == 1:
        return retlist[0]
    else:
        return retlist
...
class Library(object):
    def __init__(self, config, client, artwork, TAB_LIBRARY, album_filename,
            ...
            filtering_entry_revert_color, filter_key_pressed, on_add_item,
            connected, on_library_button_press, new_tab):

        self.consts = consts.Constants()
...
#		self.searchbox.pack_start(gtk.VSeparator(), False, False, 2)
        ...
        self.harddiskpb = self.library.render_icon(gtk.STOCK_HARDDISK,
                gtk.ICON_SIZE_MENU)
        self.albumpb = gtk.gdk.pixbuf_new_from_file_at_size(album_filename,
                self.consts('LIB_COVER_SIZE'), self.consts('LIB_COVER_SIZE'))
        ...
        self.VIEWS = [
            (self.consts('VIEW_FILESYSTEM'), 'filesystem',
             gtk.STOCK_HARDDISK, _("Filesystem")),
            (self.consts('VIEW_ALBUM'), 'album',
             'album', _("Albums")),
            (self.consts('VIEW_ARTIST'), 'artist',
             'artist', _("Artists")),
            (self.consts('VIEW_GENRE'), 'genre',
        ...
        self.libfilter_changed_handler = self.searchtext.connect('changed',
                self.libsearchfilter_feed_loop)
        searchcombo_changed_handler = self.searchcombo.connect('changed',
                self.on_library_search_combo_change)
        ...
        self.librarydata = gtk.ListStore(gtk.gdk.Pixbuf,
    ...
    def on_libraryview_chosen(self, action):
        ...
        if action.get_name() == 'filesystemview':
            self.config.lib_view = self.consts('VIEW_FILESYSTEM')
        elif action.get_name() == 'artistview':
            self.config.lib_view = self.consts('VIEW_ARTIST')
        elif action.get_name() == 'genreview':
            self.config.lib_view = self.consts('VIEW_GENRE')
        elif action.get_name() == 'albumview':
            self.config.lib_view = self.consts('VIEW_ALBUM')
    ...
    def on_library_scrolled(self, _widget, _event):
        try:
            # Use gobject.idle_add so that we can get the visible state
            # of the treeview
    ...
    def _on_library_scrolled(self):
        ...
        self.artwork.library_artwork_update(self.librarydata,
                start_row, end_row, self.albumpb)
    ...
    def library_browse(self, _widget=None, root=None):
        ...
        if root is None or\
                (self.config.lib_view == self.consts('VIEW_FILESYSTEM') and\
                self.library_get_data(root, 'path') is None):

            root = self.library_set_data(path="/")

        if self.config.wd is None or\
                (self.config.lib_view == self.consts('VIEW_FILESYSTEM') and\
                self.library_get_data(self.config.wd, 'path') is None):
        ...
        elif (self.config.lib_view == self.consts('VIEW_FILESYSTEM') and\
                root != self.config.wd)\
                or (self.config.lib_view != self.consts('VIEW_FILESYSTEM') and\
        ...
        bd = []
        while len(bd) == 0:
            if self.config.lib_view == self.consts('VIEW_FILESYSTEM') :
                bd = self.library_populate_filesystem_data(self.library_get_data(self.config.wd,
                    'path'))
            elif self.config.lib_view == self.consts('VIEW_ALBUM'):
            ...
            elif self.config.lib_view == self.consts('VIEW_ARTIST'):
            ...
            elif self.config.lib_view == self.consts('VIEW_GENRE'):
                genre, artist, album, year = self.library_get_data(self.config.wd,
                        'genre', 'artist', 'album', 'year')
                if None not in (genre, artist, album,):
                    bd = self.library_populate_data(genre=genre,
                            artist=artist, album=album, year=year)
                elif None not in (genre, artist,):
    ...
    def update_breadcrumbs(self):
        ...
        b = ui.button(img=gtk.image_new_from_stock(gtk.STOCK_FIND, 1), can_focus=False, relief=gtk.RELIEF_NONE)
        ...
        if view == self.consts('VIEW_FILESYSTEM'):
            ...
            if view == self.consts('VIEW_ALBUM'):
    ...
    def library_populate_add_parent_rows(self):
        ...
        if self.config.lib_view == self.consts('VIEW_FILESYSTEM'):
            bd = [('0',
                [self.harddiskpb, self.library_set_data(path='/'),
                    '/'])]
            ...
            for item in mpdh.call(self.client, 'lsinfo', path):
                if 'directory' in item:
                    ...
                    data = self.library_set_data(path=mpdh.get(item,
                        'directory'))
                    bd += [('d' + unicode(name).lower(),
                        [self.openpb, data, misc.escape_html(name)])]
                elif 'file' in item:
    ...
    def library_get_toplevel_cache(self, genreview=False,
            artistview=False, albumview=False):
        ...
        for _sort, info in bd:
            pb = info[0]
            if pb == self.albumpb:
                artist, album, path = self.library_get_data(info[1],
                        'artist', 'album', 'path')
                key = self.library_set_data(path=path,
        ...
        if genreview or artistview:
            ...
            for item in albums:
                ...
                if num_songs > 0:
                    ...
                    if artist and year and len(artist) > 0 and\
                            len(year) > 0 and artist != self.NOTAG and\
                            year != self.NOTAG:
                    ...
                    display += self.add_display_info(num_songs,
                            int(playtime) / 60)
                    bd += [(misc.lower_no_the(album),
                        [self.albumpb, data, display])]
    ...
    def list_identify_VA_albums(self, albums):
        for i in range(len(albums)):
            if i + self.consts('NUM_ARTISTS_FOR_VA') - 1 > len(albums) - 1:
                # Get rid of the breaks !!!!!
                break
            VA = False
            for j in range(1, self.consts('NUM_ARTISTS_FOR_VA')):
                ...
                if j == self.consts('NUM_ARTISTS_FOR_VA') - 1:
                    VA = True
            if VA:
                album, year, path = self.library_get_data(albums[i],
                        'album', 'year', 'path')
                artist = self.VAstr
                albums[i] = self.library_set_data(album=album,
                        artist=artist, year=year, path=path)
                j = 1
        ...
        if genre is not None and artist is None and album is None:
            ...
            if len(artists) > 0:
                ...
                for artist in artists:
                    playtime, num_songs = self.library_return_count(genre=genre,
                            artist=artist)
                    if num_songs > 0:
            ...
            # Albums first:
            if genre is not None:
                albums = self.library_return_list_items('album',
                ...
                for year in years:
                    if genre is not None:
                        playtime, num_songs = self.library_return_count(genre=genre,
                                artist=artist, album=album, year=year)
                        if num_songs > 0:
                            files = self.library_return_list_items('file',
                                    genre=genre, artist=artist, album=album,
                                    year=year)
            ...
            bd += self.library_populate_data_songs(genre,
                    artist, self.NOTAG, None)
        else:
    ...
    def library_populate_data_songs(self, genre, artist, album, year):
        bd = []
        if genre is not None:
            songs, _playtime, _num_songs = self.library_return_search_items(genre=genre,
                    artist=artist, album=album, year=year)
        else:
            songs, _playtime, _num_songs = self.library_return_search_items(artist=artist,
                    album=album, year=year)
        for song in songs:
            ...
            try:
                bd += [('f' + disc + track + misc.lower_no_the(mpdh.get(song, 'title')),
                    [self.sonatapb, data, formatting.parse(self.config.libraryformat, song, True)])]
            except:
                # TODO add exceptions types
                bd += [('f' + disc + track + unicode(mpdh.get(song, 'file')).lower(),
                    [self.sonatapb, data, formatting.parse(self.config.libraryformat, song, True)])]
        return bd
    ...
    def library_return_list_items(self, itemtype, genre=None, artist=None,
        ...
        if len(searches) > 0:
            for s in searches:
                ...
                if '' in s:
                    items = []
                    songs, playtime, num_songs = self.library_return_search_items(genre,
                            artist, album, year)
                    for song in songs:
    ...
    def library_return_count(self, genre=None, artist=None,
        ...
        searches = self.library_compose_list_count_searchlist(genre,
                artist, album, year)
        playtime = 0
        num_songs = 0
        for s in searches:
            ...
            if '' in s and mpdh.mpd_major_version(self.client) <= 0.13:
                ...
                _results, playtime, num_songs = self.library_return_search_items(genre=genre,
                        artist=artist, album=album, year=year)
    ...
    def library_compose_list_count_searchlist(self, genre=None, artist=None,
            album=None, year=None):
        s = []
        s, self.lib_list_genres = self.library_compose_list_count_searchlist_single(genre,
                'genre', self.lib_list_genres, s)
        if s is None:
            return []
        s, self.lib_list_artists = self.library_compose_list_count_searchlist_single(artist,
                'artist', self.lib_list_artists, s)
        if s is None:
            return []
        s, self.lib_list_albums = self.library_compose_list_count_searchlist_single(album,
                'album', self.lib_list_albums, s)
        if s is None:
            return []
        s, self.lib_list_years = self.library_compose_list_count_searchlist_single(year,
                'date', self.lib_list_years, s)
        if s is None:
            return []
        return s
    ...
    def library_return_search_items(self, genre=None, artist=None,
        ...
        for s in searches:
            ...
            if '' in s and mpdh.mpd_major_version(self.client) <= 0.13:
                ...
                # Can't search for empty tags,
                # search broader and filter instead:
            ...
            if items is not None:
                for item in items:
                    if strip_type is None or\
                            (strip_type is not None and\
                            not strip_type in item.keys()):

                        match = True
                        pos = 0
                        # Ensure that if, e.g.,
                        # "foo" is searched, "foobar" isn't returned too
                        for arg in args_tuple[::2]:
                            if arg in item and\
                                    unicode(mpdh.get(item, arg)).upper() != unicode(args_tuple[pos + 1]).upper():
                                match = False
                                break
                            pos += 2
                        if match:
    ...
    def library_set_view(self, select_items=True):
        ...
        try:
            if self.config.wd in self.libraryposition:
                self.library.scroll_to_point(-1,
    ...
    def library_get_data_level(self, data):
        if self.config.lib_view == self.consts('VIEW_FILESYSTEM'):
            ...
            # the path:
            level = 0
            album, artist, genre, year = library_get_data(data, 'album',
                    'artist', 'genre', 'year')
    ...
    def on_library_query_tooltip(self, widget, x, y, keyboard_mode, tooltip):
        ...
        if not pathinfo:
            ...
            # shows up again:
            gobject.idle_add(self.library_search_tooltips_enable,
                    widget, x, y, keyboard_mode, None)
            return False
        treepath, _col, _x2, _y2 = pathinfo

        i = self.librarydata.get_iter(treepath[0])
        path = misc.escape_html(self.library_get_data(self.librarydata.get_value(i, 1),
            'path'))
        ...
        if new_tooltip != self.libsearch_last_tooltip:
            ...
            gobject.idle_add(self.library_search_tooltips_enable, widget,
                    x, y, keyboard_mode, tooltip)
    ...
    def library_search_tooltips_enable(self, widget, x, y,
    ...
    def library_get_parent(self):
        if self.config.lib_view == self.consts('VIEW_ALBUM'):
            value = self.library_set_data(path="/")
        elif self.config.lib_view == self.consts('VIEW_ARTIST'):
            album, artist = self.library_get_data(self.config.wd,
                    'album', 'artist')
            if album is not None:
            ...
            album, artist, genre = self.library_get_data(self.config.wd,
                    'album', 'artist', 'genre')
            if album is not None:
    ...
    def get_path_child_filenames(self, return_root, selected_only=True):
        ...
        for path in rows:
            ...
            value = model.get_value(i, 2)
            if value != ".." and value != "/":
                album, artist, year, genre, path = self.library_get_data(data,
                        'album', 'artist', 'year', 'genre', 'path')
                if None in (path, album, artist, year, genre,):
                    if pb == self.sonatapb:
                        # File
                        items.append(path)
                    else:
                    ...
                    results, _playtime, _num_songs = self.library_return_search_items(genre=genre,
                            artist=artist, album=album, year=year)
                    for item in results:
    ...
    def libsearchfilter_feed_loop(self, editable):
        ...
        try:
            ...
            # TODO add exceptions types
            pass
        self.libfilterbox_source = gobject.timeout_add(300,
                self.libsearchfilter_start_loop, editable)
    ...
    def libsearchfilter_do_search(self, searchby, todo):
        if not self.prevlibtodo_base in todo:
            ...
            self.prevlibtodo_base_results = mpdh.call(self.client, 'search',
        ...
        # because we'll be searching for a match
        # in items that are also escaped.
        #
