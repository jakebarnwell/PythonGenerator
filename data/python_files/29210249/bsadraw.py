...
def plot_raw(x,y,color='k',size=1, marker='.', **kw):
    ...
    """
    pyplot.plot(x, y, marker='.', ls='None', markersize=size,color=color, **kw)
...
def plotG_one(i, gresults, chromlens, drawraw=True,
        ...
        smoothcolor='r', smoothwidth=1.5, ymax=None, maxgap=10000):
...
def plotG_all(gresults, chromlens,  drawraw=True,
    ...
    fig = setup_many_chromfig(chromlens, ymax, 
            chrbounds=chrbounds, boundstyle=boundstyle, chrfontsize=8)
    X = gresults.coords
    if drawraw:
