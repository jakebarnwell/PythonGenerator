...
class Info(object):
    def __init__(self, config, info_image, linkcolor, on_link_click_cb,
            get_playing_song, TAB_INFO, on_image_activate, on_image_motion_cb,
            on_image_drop_cb, album_return_artist_and_tracks, new_tab):
    ...
    def _update_song(self, songinfo):
        ...
        path = misc.file_from_utf8(os.path.join(self.config.musicdir[self.config.profile_num],
            mpdh.get(songinfo, 'file')))
        if os.path.exists(path):
            filelabel.set_text(os.path.join(self.config.musicdir[self.config.profile_num],
                mpdh.get(songinfo, 'file')))
    ...
    def _check_for_local_lyrics(self, artist, title, song_dir):
        locations = [self.consts('LYRICS_LOCATION_HOME'),
                self.consts('LYRICS_LOCATION_PATH'),
                self.consts('LYRICS_LOCATION_HOME_ALT'),
                self.consts('LYRICS_LOCATION_PATH_ALT')]
    ...
    def get_lyrics_start(self, search_artist, search_title,
    ...
    def get_lyrics_response(self, artist_then, title_then, song_dir,
        ...
        if lyrics and not error:
            filename = self.target_lyrics_filename(artist_then,
                    title_then, song_dir)
            # Save lyrics to file:
    ...
    def target_lyrics_filename(self, artist, title, song_dir,
        ...
        file_map = {
            self.consts('LYRICS_LOCATION_HOME'): ('~/.lyrics', '%s-%s.txt'),
            self.consts('LYRICS_LOCATION_PATH'):\
                    (self.config.musicdir[self.config.profile_num],
                        song_dir, '%s-%s.txt'),

            self.consts('LYRICS_LOCATION_HOME_ALT'): ('~/.lyrics', '%s - %s.txt'),

            self.consts('LYRICS_LOCATION_PATH_ALT'):\
                    (self.config.musicdir[self.config.profile_num],
                        song_dir, '%s - %s.txt'),
               }
        return misc.file_from_utf8(misc.file_exists_insensitive(os.path.expanduser(os.path.join(*file_map[lyrics_loc])) %\
                (artist, title)))
