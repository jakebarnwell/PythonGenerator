    ...
    def apply_profile_arg(self, config):
        if self.arg_profile:
            ...
            if a > 0 and a <= len(config.profile_names):
                ...
                aprint(3, _('Starting Sonata with profile %s...') %\
                        config.profile_names[config.profile_num])
            else:
                aprint(1, _('%d is not an available profile number.') % a)
                aprint(1, _('Profile numbers must be between 1 and %d.') %\
                        len(config.profile_names))
                sys.exit(1)


    def verbose_arg(self) :
        
        infile = False
        a = 2
        isonterm = True
        if self.arg_verbose != None :
            if a >= 0 and a <= 3:
                a = self.arg_verbose

        if self.arg_infile :
            isonterm = False
            infile = self.arg_infile

        if not output.isatty():
            isonterm = False
        
        output.reinit(a, isonterm, infile)

        aprint(3, _('Starting Sonata with verbose set to %s (%d)') % (output.level(), a)) 
