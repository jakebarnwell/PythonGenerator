...
from pyrem_strs.config import _SLOTS_VALID_IN_STR_DERIVED
...
if hasattr((), "to_rowcol"):
    raise ImportError("module pyrem_strs assumes that tuple type doesn't have a method to_rowcol")
...
class NamedStr(str):
    ...
    if _SLOTS_VALID_IN_STR_DERIVED: __slots__ = [ '__rowIndicesTuple', '__name' ]
    
    def __getvalue(self): return self.__str__()
    ...
    @staticmethod
    def new_from_file(filePath, mode=None):
        """
    ...
    def __eq__(self, right):
        return isinstance(right, NamedStr) and \
            self.__name == right.name and str.__eq__(self, right)
    ...
    def __getitem__(self, index):
        return NamedStr(self.__name, str.__getitem__(self, index))

    def __getslice__(self, beg, end):
        return NamedStr(self.__name, str.__getslice__(self, beg, end))
    ...
    def get_rowcols(self):
        ...
        return _ut.row_index_list_to_row_col_table( \
                self.__rowIndicesTuple, self.__str__())
...
def origined_str(something):
    ...
    if isinstance(something, ( list, tuple )):
        ...
        return OriginedStr.from_origin(something)
    raise TypeError("Can't cast to pyrem_strs.originedstr.OriginedStr")

_ReferenceRange_optimized = ReferenceRange.optimized

class OriginedStr(str): # immutable
    """
    ...
    if _SLOTS_VALID_IN_STR_DERIVED: __slots__ = [ '__references' ]
    ...
    def get_origin(self):
        ...
        if isinstance(self.__references, ReferenceRange):
    ...
    def from_origin(referenceRanges):
        ...
        if not referenceRanges:
            ...
            return OriginedStr("".join(rr.to_string() for rr in referenceRanges), tuple(referenceRanges))
    ...
    #def __eq__(self, right):
    #    return str.__eq__(self, right)
    
    #def __ne__(self, right):
    #    return str.__ne__(self, right)
    ...
    def join(self, strs):
        if not strs: return OriginedStr("")
        if self:
    ...
    def partition(self, sep):
        ...
        if i >= 0:
            ...
            return ( self, OriginedStr(''), OriginedStr('') )
    ...
    def rpartition(self, sep):
        ...
        if i >= 0:
            ...
            return ( OriginedStr(''), OriginedStr(''), self )
...
class FileTable(UnoverwritableDict):
    ...
    def read(self, filePath, mode=None):
        """
        ...
        if filePath not in self:
            self[filePath] = NamedStr.new_from_file(filePath, mode=mode)
        return self[filePath]
    
    def read_from_directory(self, dirPath, filePathPattern, recursive=False, mode=None):
        """
